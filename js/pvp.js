...
(() => {
  ...
  let bullets = [];
  let missiles = [];
  // Confirmaciones de láser ya aplicadas. Evita descontar dos veces una misma ráfaga.
  const confirmedLaserHits = new Set();
  let impactFx = [];
  let asteroidFx = [];
  let hitFlashUntil = 0;
  let hitShakeUntil = 0;
  ...
  function qaBotCups(){
    const level=qaBotRankLevel();
    if(level===null)return getPvpCups();
  ...
  if(qaBotRankSelect){
    ...
    qaBotRankSelect.addEventListener('change',()=>{
      if(!qaBotRankOverrideAllowed){qaBotRankSelect.value='auto';return;}
  ...
  let queueWaitingCount=1, queueMaxWaitMs=0;
  ...
  async function findMatch(partyCode=''){
    ...
    ws.addEventListener('message',event=>{
      ...
      if(m.type==='queue-waiting'){
        ...
        if(Number.isFinite(Number(m.maxWaitMs))&&Number(m.maxWaitMs)>0)queueMaxWaitMs=Number(m.maxWaitMs);
  ...
  function resetArena(){
    ...
    for(const [slot,state] of peerStates){
      const pos=pvpMode==='arena10'?arena10Starts[(slot-1)%10]:(pvpMode==='arena'||pvpMode==='arena10')?arenaStarts[(slot-1)%5]:starts[(slot-1)%4];state.x=state.targetX=pos[0];state.y=state.targetY=pos[1];
      const peerPlayer=players.find(p=>Number(p.slot)===slot)||{};state.lives=pvpShipStats(peerPlayer.ship||'Gallina').maxLives;
      const team=Number(peerPlayer.team||0);state.angle=state.targetAngle=team===2?Math.PI/2:-Math.PI/2;state.visualAngle=state.targetVisualAngle=state.angle;
    }
    if(pvpMode==='1v1'){
  ...
  function qaCheckLocalResult(){
    if(!qaPvpActive||matchFinished)return;
    const alive=players.filter(p=>!eliminated.has(Number(p.slot)));
    if(pvpMode==='2v2'){
      const myAlive=alive.some(p=>Number(p.team)===myTeam);
      const enemyAlive=alive.some(p=>Number(p.team)!==myTeam);
      if(!enemyAlive)return endArena('🏆 QA · victoria de equipo','win');
      if(!myAlive)return endArena('💥 QA · equipo eliminado','loss');
      return;
    }
    if(pvpMode==='arena'||pvpMode==='arena10'){
      if(meEliminated&&!qaPvpPlacement)qaPvpPlacement=alive.length+1;
      if(alive.length<=1){
        const winner=alive[0];
        const won=Number(winner?.slot)===mySlot;
        return endArena(won?'🏆 QA · victoria en Arena':'🌌 QA · Arena finalizada',won?'win':'loss',won?1:(qaPvpPlacement||players.length));
      }
    }
  }
  ...
  function handlePeer(p,fromSlot=0,fromTeam=0){
    ...
    // antes de descartar mensajes del propio slot, porque la vida sólo cambia
    // después de que el servidor haya validado la confirmación.
    if(p.type==='hit-confirm'){
      const kind=p.attackKind==='missile'?'missile':'laser';
      if(kind==='laser'){
        const confirmedShotId=String(p.shotId||'');
        const targetSlot=Number(p.targetSlot||0);
        const attackerSlot=Number(p.attackerSlot||0);
        const rawX=Number(p.hitX),rawY=Number(p.hitY),hasPoint=Number.isFinite(rawX)&&Number.isFinite(rawY);
        // hitX/hitY están en el sistema local del defensor. En 1v1 el otro
        // dispositivo ve ese sistema rotado 180 grados.
        const needsMirror=pvpMode==='1v1'&&targetSlot!==mySlot;
        const hx=hasPoint&&needsMirror?worldWidth-rawX:rawX;
        const hy=hasPoint&&needsMirror?worldHeight-rawY:rawY;
        const candidates=bullets.filter(b=>b.life>0&&String(b.shotId||'')===confirmedShotId);
        for(const hit of candidates){hit.life=0;if(hasPoint){hit.x=hx;hit.y=hy;}}
        if(hasPoint)impactFx.push({x:hx,y:hy,life:.32,maxLife:.32});

        if(targetSlot===mySlot && confirmedShotId && !confirmedLaserHits.has(confirmedShotId)){
          confirmedLaserHits.add(confirmedShotId);
          lastRegenAt=performance.now();
          lastAttackerSlot=attackerSlot;
          lastAttackKind='laser';
          meState.lives=Math.max(0,meState.lives-1);
          hitFlashUntil=performance.now()+220;
          hitShakeUntil=performance.now()+150;
          updateLives();
          if(meState.lives<=0){
            send({type:'defeat',slot:mySlot,team:myTeam,killerSlot:lastAttackerSlot||0,attackKind:'laser'});
    ...
    if(p.type==='state'){
      ...
      spawnRemoteShot(mirrorX(Number(p.x)),mirrorY(Number(p.y)),mirrorAngle(Number(p.angle)),p.ship,fromSlot,fromTeam,Number(p.targetSlot||0),String(p.shotId||''),Number(p.firedAt||0));
    } else if(p.type==='missile'){
      spawnRemoteMissile(mirrorX(Number(p.x)),mirrorY(Number(p.y)),p.ship,p.missileType,p.isPro,fromSlot,fromTeam,Number(p.targetSlot||0),Number(p.curveSide||0));
    }
  }
  ...
  function shoot(){
    ...
    send({type:'shot',shotId,targetSlot:Number(targetPlayer.slot),firedAt:Date.now(),x:sx,y:sy,angle:sa,ship:myShip});
  }
  function spawnRemoteShot(x,y,a,ship,ownerSlot=0,ownerTeam=0,targetSlot=0,shotId='',firedAt=0){
    if(!Number.isFinite(x+y+a))return;
    const sideX=Math.cos(a+Math.PI/2)*9,sideY=Math.sin(a+Math.PI/2)*9;
    [-1,1].forEach((s,i)=>{const bx=x+sideX*s,by=y+sideY*s;bullets.push({x:bx,y:by,prevX:bx,prevY:by,vx:Math.cos(a)*840,vy:Math.sin(a)*840,angle:a,ship:ship||'Gallina',own:false,ownerSlot,ownerTeam,targetSlot:Number(targetSlot||0),shotId:String(shotId||''),firedAt:Number(firedAt||0),beamIndex:i,life:1.5});});
  }
  ...
  function update(dt,now){
    ...
    if(botMatch&&(pvpMode==='1v1'||pvpMode==='2v2'||(pvpMode==='arena'||pvpMode==='arena10')||pvpMode==='arena10')){
      for(const bp of players.filter(p=>p.bot&&!eliminated.has(Number(p.slot)))){
        ...
        const botStats=pvpShipStats(bp.ship||'Gallina');
        if(st.lives>0&&st.lives<botStats.maxLives&&now-hit>=botStats.regenDelay&&now-regen>=botStats.regenEvery){
    ...
    if(botMatch&&(pvpMode==='1v1'||pvpMode==='2v2'||(pvpMode==='arena'||pvpMode==='arena10')||pvpMode==='arena10')&&running&&!matchFinished){
      ...
      for(const botPlayer of activeBots){
        ...
        const allyToProtect=pvpMode==='2v2'
          ? players.filter(p=>Number(p.slot)!==Number(botPlayer.slot)&&Number(p.team)===Number(botPlayer.team)&&!eliminated.has(Number(p.slot))).map(p=>({p,state:Number(p.slot)===mySlot?meState:peerFor(p.slot)})).sort((a,b)=>{
              const ar=Number(a.state.lives||20)/pvpShipStats(a.p.ship||'Gallina').maxLives,br=Number(b.state.lives||20)/pvpShipStats(b.p.ship||'Gallina').maxLives;return ar-br;
            })[0]
          : null;
        ...
        const priorTarget=ai.targetSeen;
        const targetDt=priorTarget&&Number(priorTarget.slot)===Number(chosen?.p?.slot)?Math.max(.016,Math.min(.35,(now-Number(priorTarget.at||now))/1000)):0;
        const targetVx=targetDt?(chosen.state.x-Number(priorTarget.x||chosen.state.x))/targetDt:0;
        const targetVy=targetDt?(chosen.state.y-Number(priorTarget.y||chosen.state.y))/targetDt:0;
        if(chosen)ai.targetSeen={slot:Number(chosen.p.slot),x:chosen.state.x,y:chosen.state.y,at:now};
        const shotAim=()=>{
          if(!chosen)return trueAim;
          const lead=[0,0,0,.05,.09,.13,.17][rankLevel]||0;
          const error=[.22,.18,.14,.11,.075,.045,.025][rankLevel]||.22;
          return Math.atan2(chosen.state.y+targetVy*lead-bot.y,chosen.state.x+targetVx*lead-bot.x)+(Math.random()-.5)*2*error;
        };
        ...
        if(botZone?.active&&Math.hypot(bot.x-botZone.cx,bot.y-botZone.cy)>botZone.radius){
          ...
          if(now-lastZone>=1000){
            ...
            if(bot.lives<=0&&!eliminated.has(Number(botPlayer.slot))&&!pendingBotDefeats.has(Number(botPlayer.slot))){
              ...
              if(qaPvpActive){markEliminated(Number(botPlayer.slot));pendingBotDefeats.delete(Number(botPlayer.slot));qaCheckLocalResult();}
              else send({type:'bot-defeat',slot:Number(botPlayer.slot),team:0,killerSlot:0,attackKind:'zone'});
            }
          }
        }
        ...
        const targetLifeRatio=chosen?Math.max(0,Number(chosen.state.lives||20))/pvpShipStats(chosen.p.ship||'Gallina').maxLives:1;
        const missileWorth=rankLevel<3||targetLifeRatio<=.72||(rankLevel>=5&&dist>=170);
        if(targetInAttackRange&&missileWorth&&now>=ai.nextMissileAt){
      ...
      const botAuthority=humanSlots.length===0||mySlot===Math.min(...humanSlots);
      {
        const damageBot=(targetPlayer,kind,killerSlot,hitX,hitY)=>{
          ...
          if(st.lives<=0&&!eliminated.has(slot)&&!pendingBotDefeats.has(slot)){
            ...
            if(qaPvpActive){
              markEliminated(slot);pendingBotDefeats.delete(slot);
              if(Number(killerSlot)===mySlot){matchKills++;matchBotKills++;}
              qaCheckLocalResult();
            }else send({type:'bot-defeat',slot,team:Number(targetPlayer.team||0),killerSlot:Number(killerSlot||0),attackKind:kind});
          }
          return true;
        };
        for(const b of bullets){
  ...
  function draw(){
    ...
    for(const fx of impactFx){
      const t=Math.max(0,fx.life/fx.maxLife),r=fx.predicted?3+(1-t)*7:7+(1-t)*30;
      arenaCtx.save();arenaCtx.globalAlpha=Math.min(1,t*1.7);
      const color=fx.predicted?'#fde68a':'#fff',rays=fx.predicted?4:8,rayLength=fx.predicted?7+(1-t)*7:10+(1-t)*24;
      arenaCtx.fillStyle=color;arenaCtx.beginPath();arenaCtx.arc(fx.x,fx.y,(fx.predicted?2:7)*t+3,0,Math.PI*2);arenaCtx.fill();
      arenaCtx.strokeStyle=color;arenaCtx.lineWidth=fx.predicted?1.25:4;arenaCtx.beginPath();arenaCtx.arc(fx.x,fx.y,r,0,Math.PI*2);arenaCtx.stroke();
      for(let i=0;i<rays;i++){const a=i*Math.PI*2/rays,len=rayLength;arenaCtx.beginPath();arenaCtx.moveTo(fx.x+Math.cos(a)*4,fx.y+Math.sin(a)*4);arenaCtx.lineTo(fx.x+Math.cos(a)*len,fx.y+Math.sin(a)*len);arenaCtx.stroke();}
      arenaCtx.restore();
    }
  ...
  window.GallinaPvp={
    ...
    startQaMatch(mode='1v1',speed=1){
      ...
      qaPvpActive=true;qaPvpSpeed=Math.max(1,Math.min(8,Number(speed)||1));qaPvpStartedAt=performance.now();qaPvpSimMs=0;qaPvpPlacement=0;
